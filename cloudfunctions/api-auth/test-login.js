...
exports.main = async (event) => {
  ...
  try {
    const users = await db.collection('users')
      ...
      .limit(1)
      .get()
    results.push('users查询: ' + JSON.stringify(users.data?.map(u => ({id:u._id, username:u.username, role:u.role}))))
  ...
  // 3. 测试add
  try {
    const r = await db.collection('sessions').add({ data: { token: 'test123', userId: 'test', expireAt: Date.now(), platform: 'test', createdAt: new Date().toISOString() } })
    results.push('sessions add成功: ' + r.id)
    // 清理
    await db.collection('sessions').doc(r.id).remove()
    results.push('sessions 删除成功')
