...
export const CloudCommentService = {
  ...
  async getCourseComments(courseId: string, params?: { limit?: number; offset?: number }): Promise<{
    ...
    try {
      ...
      const data = await (dbService.getAll as any)(this.collection, query, { limit, skip: offset })
  ...
  async getUserComments(userId?: string): Promise<Comment[]> {
    try {
      ...
      const data = await (dbService.getAll as any)(this.collection, { userId: targetUserId }, { limit: 50 })
